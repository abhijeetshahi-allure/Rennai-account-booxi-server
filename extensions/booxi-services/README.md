...
## Setup: Booxi API backend URL (required for CORS)

The extension fetches Booxi data from your app backend. To avoid CORS errors, configure the **Booxi API backend URL**:

1. Run `shopify app dev` and note the **App URL** in the terminal (e.g. `https://xxx.trycloudflare.com`).
2. In Shopify Admin: **Content** → **Customer accounts** → **Customize**.
3. Open the **Booxi Service Bookings** extension settings.
4. Set **Booxi API backend URL** to your App URL.
5. Save. Restart dev if the tunnel URL changes.

For production, use your deployed backend URL (e.g. from Railway, Render).
